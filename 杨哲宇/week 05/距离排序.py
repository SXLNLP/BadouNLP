...
def main():
    ...
    # 计算并排序每个簇内点到质心的距离
    cluster_distances = calculate_distances(vectors, kmeans.labels_, kmeans.cluster_centers_)

    # 计算每个簇的平均距离用于排序
    cluster_avgdistances = {
        label: sum(dist for dist, _ in distances) / len(distances)
        for label, distances in cluster_distances.items()
    }

    # 按平均距离从大到小排序簇
    sorted_clusters = sorted(cluster_avgdistances.items(), key=lambda x: x[1], reverse=True)
