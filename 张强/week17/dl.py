...
class DialogueSystem:
    def __init__(self):
        self.load()
        self.rehear_intent = ["你说什么","再说一遍","我没听清"]

    def load(self):
        self.all_node_info = {}
        self.load_scenario("week17+对话系统\week17 对话系统\scenario\scenario-买衣服.json")
        # self.load_scenario("scenario-看电影.json")
        self.load_slot_templet("week17+对话系统\week17 对话系统\scenario\slot_fitting_templet.xlsx")
    ...
    def run(self, query, memory):
        ...
        '''
        max_rehear_score = 0
        for sentence in self.rehear_intent:
            score = self.calucate_sentence_score(query, sentence)
            if score > max_rehear_score:
                max_rehear_score = score
        # 保存当前回复作为下一次可能的重听内容
        if "response" in memory:
            memory["last_response"] = memory["response"]
        # 检测到重听意图
        if max_rehear_score > 0.5:
            memory["policy"] = "rehear"
...
if __name__ == '__main__':
    ...
    memory = {"available_nodes":["scenario-买衣服_node1"],
              "last_response": ""  # 用于存储上一次回复，供重听功能使用
              } #用户状态
    while True:
        query = input("请输入：")
        # query = "你好"    
